...
// serial
static constexpr std::string_view SerialPortKey{ "serialPort" };
static constexpr std::string_view SerialEnableKey{ "serialEnable" };
...
#undef PROFILE_SETTINGS_TO_JSON
    ...
    const auto profile = winrt::make_self<Profile>();
    profile->_Name = _Name;
    profile->_SerialEnable = _SerialEnable;
    profile->_BoundRate = _BoundRate;
    profile->_DataWidth = _DataWidth;
    profile->_CheckBit = _CheckBit;
    profile->_StopBit = _StopBit;
    profile->_FlowControl = _FlowControl;
    profile->_Commandline = _Commandline;
    profile->_SerialPort = _SerialPort;
    if (profile->SerialEnable() && profile->_Commandline)
    {
    #define SERIAL_SETTING(_map, _curr, r_and_sct, val)                    \
        do                                                                 \
        {                                                                  \
            auto curr = _curr;                                             \
            auto map = _map;                                               \
            ...
            {                                                              \
                if (value == curr)                                         \
                {                                                          \
                    std::wstring_view _key{ key };                         \
                    auto fmtKey = fmt::format(L"{}{}{}/{}",                \
                                              r_and_sct,                   \
                                              char(std::towupper(key[0])), \
                                              _key.substr(1),              \
                                              L"Content");                 \
                    auto cmd = GetLibraryResourceString(fmtKey);           \
                    val = (val + L" --" + cmd);                            \
                    break;                                                 \
                }                                                          \
            }                                                              \
        ...
        winrt::hstring args;
        args = (args + L" --" + profile->SerialPort());
        SERIAL_SETTING(winrt::Microsoft::Terminal::Settings::Model::EnumMappings::SerialBoundRate(),
                       profile->BoundRate(),
                       std::wstring_view{ L"Profile_BoundRate" },
                       args);
        SERIAL_SETTING(winrt::Microsoft::Terminal::Settings::Model::EnumMappings::SerialDataWidth(),
                       profile->DataWidth(),
                       std::wstring_view{ L"Profile_DataWidth" },
                       args);
        SERIAL_SETTING(winrt::Microsoft::Terminal::Settings::Model::EnumMappings::SerialCheckBit(),
                       profile->CheckBit(),
                       std::wstring_view{ L"Profile_CheckBit" },
                       args);
        SERIAL_SETTING(winrt::Microsoft::Terminal::Settings::Model::EnumMappings::SerialStopBit(),
                       profile->StopBit(),
                       std::wstring_view{ L"Profile_StopBit" },
                       args);
        SERIAL_SETTING(winrt::Microsoft::Terminal::Settings::Model::EnumMappings::SerialFlowControl(),
                       profile->FlowControl(),
                       std::wstring_view{ L"Profile_FlowControl" },
                       args);

        auto val = winrt::to_string(*profile->_Commandline);
        size_t pos = val.find(" --");
        if (pos != val.npos)
        {
            auto cmd = val.substr(0, pos);
            winrt::hstring cmdline = winrt::to_hstring(cmd) + args;
            profile->Commandline(cmdline);
        }
        else
        {
            profile->Commandline(profile->Commandline() + args);
        }
        #undef SERIAL_SETTING
    }
    else if (!profile->SerialEnable() && _Commandline)
    {
        auto val = winrt::to_string(*_Commandline);
        size_t pos = val.find(" --");
        if (pos != val.npos)
        {
            auto cmd = val.substr(0, pos);
            profile->Commandline(winrt::to_hstring(cmd));
        }
    }
    JsonUtils::SetValueForKey(json, "commandline", profile->Commandline());
